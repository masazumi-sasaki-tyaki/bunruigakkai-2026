...
def main():
    ...
    for scenario_name, settings in ANALYSIS_SCENARIOS.items():
        ...
        if simulation_results:
            ...
            # 位置を左右にずらして2つのバーを描画
            ax.bar(x - width/2, cl4_values, width, label='CL4 (定型)', color='skyblue', edgecolor='black')
            ax.bar(x + width/2, cl2_values, width, label='CL2 (非定型)', color='lightcoral', edgecolor='black')

            # 軸とラベルの設定
            ax.set_ylabel('割合 (%)')
            ax.set_title('口縁部の磨消縄文の特徴変化 (シミュレーション平均)', pad=15, fontsize=14)
            ax.set_xticks(x)
